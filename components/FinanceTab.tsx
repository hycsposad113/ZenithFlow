...
import { TrendingUp, Wallet, ArrowUpRight, ArrowDownRight, Brain, Sparkles, ChevronRight, Activity, Calendar, List, Layers, X, Trash2, Edit2 } from 'lucide-react';
            ...
            <div className="glass-card p-8 rounded-[40px] border border-white/20">
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-3">
                  <Button onClick={handleAddTransaction} variant="primary" className="flex-1 h-14 rounded-[20px] text-[12px]">
                    {editingTx ? 'Update Entry' : 'Record Entry'}
                  </Button>
                  {editingTx && (
                    <Button onClick={handleCancelEdit} variant="secondary" className="px-6 h-14 rounded-[20px] text-[12px] bg-white/10">
                      Cancel
                    </Button>
                  )}
                </div>
                ...
                <div className="md:col-span-5 space-y-4">
                  ...
                  <div className="space-y-3 overflow-y-auto pr-2 scrollbar-hide max-h-[350px]">
                    {transactions.filter(t => t.currency === activeTab).map(tx => (
                      <div key={tx.id} className={`flex justify-between items-center glass-card p-5 rounded-3xl hover:bg-white/10 transition-all group relative ${editingTx?.id === tx.id ? 'ring-2 ring-white/50 bg-white/10' : ''}`}>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              className="p-2 hover:bg-white/20 rounded-lg transition-colors text-white/60 hover:text-white"
                            >
                              <Edit2 size={14} />
                            </button>
                            <button
                              onClick={(e) => handleDeleteTransaction(tx.id, e)}
                              className="p-2 hover:bg-red-500/40 rounded-lg transition-colors text-white/60 hover:text-red-400"
                            >
                              <Trash2 size={14} />
                            </button>
                          </div>
