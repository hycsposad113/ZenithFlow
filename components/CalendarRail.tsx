          ...
          <div className="relative w-full h-full mt-0 ml-10 mr-0 pointer-events-auto">
            ...
            {layoutItems.map(({ item, top, height, left, width, zIndex }) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`item-box absolute rounded-lg p-2 text-[10px] group transition-all shadow-xl flex flex-col cursor-pointer border border-white/20 overflow-hidden ${item.status === TaskStatus.COMPLETED ? 'opacity-40 brightness-75' : 'hover:brightness-110'
