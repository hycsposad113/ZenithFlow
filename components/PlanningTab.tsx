...
interface PlanningTabProps {
  ...
  onUpdateAnalysis: (date: string, analysis: any) => void;
}

// Mantra logic now relies entirely on AI-generated advice stored in dailyAnalyses
...
export const PlanningTab: React.FC<PlanningTabProps> = ({
  ...
  selectedDate, setSelectedDate, onUpdateAnalysis
}) => {
  ...
  // Update mantra when date changes or analyses load
  useEffect(() => {
    // We try to find a task-generated advice for this day, OR fallback to previous day's insight?
    // Actually, generateDailyRitual returns advice for the CURRENT day's plan.
    // We should probably overlook how we store this.
    // Ideally, we store the 'mantra' in dailyAnalyses or a new dictionary.
    // For now, let's assume we want to pull it from a local mapping if we had one,
    // but prompts say "AI Daily Ritual generation".
    // Let's rely on the result of `generateDailyRitual` setting it into state.
    // BUT we need it to PERSIST.

    // To persist, we should probably check if we have a saved mantra derived from somewhere.
    // Since we don't have a dedicated "Mantra" field in AppState yet, let's use a temporary lookup
    // or just rely on the user generating it fresh if it's missing.
    // OR BETTER: We can repurpose `review` or `dailyAnalyses` to store it?
    // Actually, let's look at `App.tsx` state -> we have `dailyAnalyses`.
    // Maybe we can store the mantra INSIDE `dailyAnalyses` for that date?
    // The current schema is { insight, bookReference, concept, actionItem }.
    // We can add `mantra` to it.

    // For this immediate step, let's just initialize it to empty 
    // and rely on `handleMorningRitual` to set it.
    // We will need to update `dailyAnalyses` to store it permanently in the next step.
    const analysisForDay = dailyAnalyses[selectedDateStr] as any;
    if (analysisForDay && analysisForDay.mantra) {
      setMantra(analysisForDay.mantra);
    } else {
      setMantra('');
    }
  }, [selectedDateStr, dailyAnalyses]);
  ...
  const handleRoutineUpdate = (field: 'wakeTime' | 'meditation' | 'exercise', value: any) => {
    ...
    const now = new Date();
    const offsetCalc = now.getTimezoneOffset() * 60000;
    const todayStr = new Date(now.getTime() - offsetCalc).toISOString().split('T')[0];
  ...
  const handleMorningRitual = async () => {
    ...
    try {
      ...
      if (result.advice) {
        setMantra(result.advice);
        // Checking props... we DO HAVE `setDailyStats` but NOT `setDailyAnalyses`.
        // We need to add `onUpdateAnalysis` prop or similar.
      }
    } catch (e) {
      console.error(e);
    } finally {
